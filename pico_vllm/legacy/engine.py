...
class Engine:
    ...
    # '''
    # def prefill(self, input_ids: torch.Tensor, kvcache: PagedKVCache) -> torch.Tensor:
    #     with torch.no_grad():
    #         prefill_len = input_ids.shape[1]
            
    #         # 1. 提前分配 block（原来在 prefill_update 里做）
    #         kvcache._allocate_for_prefill(prefill_len)
            
    #         # 2. 构造 slot_mapping
    #         slot_mapping = kvcache.get_prefill_slot_mapping(prefill_len)  # (prefill_len,)
    ...
    # '''
    # def decode_step(self, input_ids: torch.Tensor, kv_cache: PagedKVCache) -> torch.Tensor:
    #     with torch.no_grad():
    #         kv_cache.prepare_decode_step()
    #         slot = kv_cache.get_decode_slot()
    #         slot_mapping = torch.tensor([slot], dtype=torch.int32, device=self.device)

    #         # 补上 block_table 和 context_lens
    ...
    #     kv_cache = self.kv_cache_cls(
    #         **self.kv_cache_kwargs #type: ignore
    #     )
    ...
    def step(self) -> list[tuple[int, str]]:
        ...
        for request in prefilling:
            ...
            with torch.no_grad():
                logits = self.model(
                    input_ids, kv_caches=[kv_cache],
                    is_prefill=True, slot_mapping=slot_mapping,
                )
        ...
        if decoding:
            ...
            # 2. 构造 batch slot_mapping
            slots = [r.kv_cache.get_decode_slot() for r in decoding]
            slot_mapping = torch.tensor(slots, dtype=torch.int32, device=self.device)  # (B,)
            
            # 3. batch input_ids: (B, 1)
            input_ids = torch.tensor(
                [[r.generated_ids[-1]] for r in decoding],
                dtype=torch.long, device=self.device
            )
            
            # 4. 构造 block_table 和 context_lens（还需要，paged attention kernel 要用）
