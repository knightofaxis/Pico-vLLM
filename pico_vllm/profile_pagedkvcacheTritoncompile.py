...
def measure_decode_step(model, prompt_ids, device, cfg):
    ...
    with torch.no_grad():
        for i in range(runs + 1):
            ...
            slot_mapping_d = torch.tensor([slot], dtype=torch.int32, device=device)
            position_ids_d = torch.tensor([[cache._seq_len]], dtype=torch.long, device=device)
...
def plot_results(results, save_path="profile_paged_slot_mapping.png"):
    ...
    fig.suptitle('Paged KV Cache (slot_mapping + store_kvcache kernel)', fontsize=14)
...
if __name__ == "__main__":
    ...
    with open("profile_paged_slot_mapping.json", "w") as f:
        json.dump(results, f, indent=2)
    print(f"\n数据已保存到 profile_paged_slot_mapping.json")
